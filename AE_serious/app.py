...
def login():
    ...
    if request.method == "POST":
        ...
        if not check_password_hash(pwd[2], password):
            ...
            return apology(2)
            #return redirect(url_for('apology', action = "Invalid credentials"))
...
def errorhandler(e):
    ...
    return apology(3, e.name, e.code)
...
def apology(source, name="", code=200):
    if source == 1:
    ...
    return render_template("apology.html", action=action)
